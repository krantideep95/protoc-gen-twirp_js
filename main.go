...
func (g *generator) generateFile(file *descriptor.FileDescriptorProto) *plugin.CodeGeneratorResponse_File {
	// This adds an assumption that the main proto file will be in the same output directory as
	// the file that we are generating. By doing so it should ensure that proto files that are in
	// sub directories won't break the requires statement.
	requirePath := "./" + path.Base(baseFileName(file)+".js")
	...
	g.P(`import createClient from "twirp";`)
	g.P(`// import our protobuf definitions`)
	g.P(`import * as pb from `, strconv.Quote(requirePath), `;`)
	g.P()
...
func (g *generator) generateProtobufClient(file *descriptor.FileDescriptorProto, service *descriptor.ServiceDescriptorProto) {
	...
	g.P(` */`)
	g.P(`const `, factoryName, ` = function(baseurl, extraHeaders, useJSON) {`)
	g.P(`    const rpc = createClient(baseurl, `, strconv.Quote(fullServiceName(file, service)), `, `, strconv.Quote(gen.Version), `,  useJSON, extraHeaders === undefined ? {} : extraHeaders);`)
	g.P(`    return {`)
	// for each method...
